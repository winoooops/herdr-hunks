...
`--version` は `herdr-hunks 0.0.3` を表示します。アクションの診断情報は
...
ブランチスコープで `M` を押すと、このワークツリーでレビューしたところまでのコミットを記録します。
その後のコミットが変更した行にはファイルパネルで `●` が付き、もう一度 `M` を押すと消えます。
ドットはコミットを反映し、未コミットの編集では増えません。そのため `M` ですべてのドットが消え、
編集内容は従来どおり各行の差分に表示されます。未追跡の行にはドットが付きません。

マークは状態ディレクトリの `bases.json` と同じ場所にある `marks.json` に、
ワークツリーごとに1つのコミットとして保存されます。ビューアを閉じても残り、
同じワークツリーの別のビューアとも共有されます。比較の基準は変更しません。
履歴の書き換えでマークしたコミットがブランチから外れたり、オブジェクトが削除されたりすると、
追跡済みの全行にドットを付けて通知しますが、行、差分、基準はそのまま機能します。

`B` はマークを基準として提示します（例：`reviewed (a1b2c3d · 12 min ago)`）。
選択すると、作業ツリーとマークとの差分に一覧を絞り込めます。
同じピッカーには、解決できる場合に `upstream`、`last commit`、`last 3 commits` も表示されるため、
よく使う基準を手入力する必要はありません。
