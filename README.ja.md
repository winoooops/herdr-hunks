...
有効な UTF-8 でないパスは、復号できないバイトを `\u{fffd}` で置き換えて表示します。git には置き換え後の名前が渡されるため、その差分は空になる
（またはその名前を持つファイルの差分になる）ことがあります。そのバイトだけが異なるパスは、branch スコープでは 1 行にまとめられ、
worktree スコープでは同じ種類の行（ステージ済み、未ステージ、未追跡）のときだけまとめられます。
